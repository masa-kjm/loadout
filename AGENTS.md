...
v0.5.0 is the current development contract and deliberately has no migration path from v0.4 profile or state schemas.
v0.1 and v0.2 are retired, unsupported, and have no compatibility contract with v0.5.0.
...
They must not determine v0.5.0 behavior, architecture, schemas, or compatibility decisions.

## Sources of Truth

Before changing v0.5.0 behavior, read the documents that own it:

- [`docs/README.md`](docs/README.md) explains document authority and v0.5.0 scope.
...
`docs/future/` is non-binding and may identify constraints for later work, but it does not change v0.5.0 behavior.
...
Build the v0.5.0 implementation from a narrow foundation.
Use closed `FileLink` and `FileCopy` variants; do not add a generic resource trait or plugin registry.
An isolated legacy algorithm may be mined only after it has been checked against the v0.5.0 contract; do not carry forward legacy resource models, command semantics, compatibility paths, or broad abstractions.
...
- Remove or replace a link or copy only when both Known state and current Actual observation prove the expected owned effect.
- A copy owns its recorded byte-content fingerprint, not matching unmanaged content or unsynchronized metadata.
...
- Reject v0.4 and unknown profile/state schemas before target inspection, recovery, planning, or state rewrite. Do not add a migration reader or command.
- Preserve deterministic v0.5.0 action phases and fully qualified resource-ID tie-breaking. YAML mapping order and include order are not execution-order controls.
...
- Once the v0.5.0 Rust package exists, run the relevant formatter, linter, and tests before handoff. Report checks that could not run and why.

## Project Skills

Source skills live in `.claude/skills/` and are synchronized for Codex discovery by `scripts/sync-codex-skills.sh`.

- `review-loadout-v0-2`: reviews v0.5 contracts, architecture boundaries, ownership and mutation safety, state consistency, compatibility, and required test evidence.
- `propose-loadout-change`: drafts evidence-based issue, commit, and pull-request text without mutating Git or GitHub state.
- `test-design-loadout-v0-2`: designs state-transition, crash-recovery, path-safety, durability, and Unix/Windows conformance evidence from v0.5 contracts.
- `maintain-loadout-context`: maintains v0.5 documentation, project-skill sources, and generated skill discovery copies without changing authority boundaries.
