...
This directory defines the v0.3 product.
...
- `architecture/` defines the v0.3 system model, responsibility boundaries, and non-negotiable architectural rules.
- `specs/` defines the v0.3 external contracts, including schemas, lifecycle behavior, ownership, path safety, durable state, and authoring commands.
- `development/` defines how the contracts are verified without redefining them.
- `future/` holds non-binding designs for work outside v0.3.0.
...
- Future documents may explore alternatives but must not alter v0.3.0 behavior.
